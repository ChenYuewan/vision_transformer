...
# =====================================================
# GELU² LUT (Q3.5 fixed-point, 8-bit address/data)
# =====================================================
addr_bits, data_bits = 8, 8
int_bits, frac_bits = 3, 5
x_min, n_lut = 0.0, 2 ** addr_bits
x_max = (2** int_bits) - (1 / (2** frac_bits))
step = (x_max - x_min) / (n_lut - 1)

x_vals = jnp.linspace(x_min, x_max, n_lut)

gelu_vals = 0.5 * x_vals * (1 + jnp.tanh(jnp.sqrt(2 / jnp.pi) * (x_vals + 0.044715 * x_vals ** 3)))
gelu2_vals_float = gelu_vals ** 2

scale = 2** frac_bits  # 小数部分 5 位
gelu2_vals_q = jnp.clip(jnp.round(gelu2_vals_float * scale), 0, 255).astype(jnp.uint8)

def gelu2_LUT(x):
    idx = jnp.clip(
        jnp.floor((x - x_min) / (x_max - x_min) * (n_lut - 1)).astype(jnp.int32),
        0, n_lut - 1)
